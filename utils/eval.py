...
def save_checkpoint(config, epoch, model, max_accuracy, optimizer, lr_scheduler, loss_scaler,
    ...
    for filename in os.listdir(config.data.log_path):
        if filename.endswith('.bin'):
            os.remove(os.path.join(config.data.log_path, filename))
            break

    # save_path = os.path.join(config.OUTPUT, f'ckpt_epoch_{epoch}.pth')
    save_path = os.path.join(config.data.log_path,
                             f"checkpoint_{epoch + 1}_{max_accuracy:2.3f}.bin")
...
def load_checkpoint(config, model, optimizer, scheduler, loss_scaler, log):
    ...
    if 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint:
        ...
        if config.local_rank in [-1, 0]:
            print('-' * 10,
                  f"Loaded Successfully '{config.model.resume}' Epoch {checkpoint['epoch'] + 1}".center(
                      58),
                  '-' * 10)
        ...
        if update_grad:
            if clip_grad is not None:
                ...
                self._scaler.unscale_(
                    optimizer)  # unscale the gradients of optimizer's assigned params in-place
