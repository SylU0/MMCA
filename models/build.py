...
from models.MyModel_v1 import MyModel
...
def build_models(config, num_classes):
    ...
    if config.model.type.lower() == 'resnet':
        ...
        if config.model.name.lower() == 'swin tiny':
            dim = 768
            backbone = swin_backbone_tiny(num_classes=num_classes,
                                          drop_path_rate=config.model.drop_path,
                                          img_size=config.data.img_size,
                                          window_size=config.data.img_size // 32)
        elif config.model.name.lower() == 'swin large':
            dim = 1536
            backbone = swin_backbone_large(num_classes=num_classes,
                                           drop_path_rate=config.model.drop_path,
                                           img_size=config.data.img_size,
                                           window_size=config.data.img_size // 32)
        else:
            dim = 1024
            if config.model.timm:
                ...
                backbone = swin_backbone(num_classes=num_classes,
                                         drop_path_rate=config.model.drop_path,
                                         img_size=config.data.img_size,
                                         window_size=config.data.img_size // 32)
    ...
    elif config.model.type.lower() == 'mymodel':
        dim = 1024
        if config.model.timm:
            backbone = create_model('swin_base_patch4_window12_384.ms_in22k',
                                    pretrained=False,
    ...
    if config.model.type.lower() == 'mymodel':
        model = MyModel(backbone=backbone, dim=dim, last_size=12, num_classes=num_classes,
                        num_heads=config.parameters.num_heads,
                        attn_drop_rate=config.parameters.att_drop,
                        head_drop=config.parameters.head_drop,
                        label_smooth=config.model.label_smooth,
                        proj_drop_rate=0, training=True,
                        # 关键参数如下:
                        agg_pos=True, cross_attn_pos=True,
        ...
        model = MultiPartsSampling(dim, config.data.img_size,
                                   backbone, config.parameters.parts_ratio,
                                   config.parameters.num_heads,
                                   config.parameters.fwp, config.parameters.att_drop,
                                   config.parameters.head_drop, config.parameters.parts_drop,
                                   num_classes,
                                   config.parameters.pos, config.parameters.parts_base,
                                   config.parameters.cross_layer,
                                   config.model.label_smooth, mixup=config.data.mixup,
...
def baseline_models(config, num_classes):
    ...
    if type == 'resnet':
        ...
        if config.model.name.lower() == 'swin tiny':
            model = swin_backbone_tiny(num_classes=num_classes,
                                       drop_path_rate=config.model.drop_path,
                                       img_size=config.data.img_size,
                                       window_size=config.data.img_size // 32,
                                       cross_layer=False)
            ...
            if config.model.timm:
                model = create_model('swin_base_patch4_window12_384.ms_in22k',
                                     pretrained=False,
                                     num_classes=num_classes,
                                     drop_path_rate=config.model.drop_path,
                                     cross_layer=False)
            else:
                model = swin_backbone(num_classes=num_classes,
                                      drop_path_rate=config.model.drop_path,
                                      img_size=config.data.img_size,
                                      window_size=config.data.img_size // 32,
                                      cross_layer=False)
        ...
        model = timm.models.create_model('maxvit_tiny_rw_224', pretrained=False, num_classes=200,
                                         img_size=384,
                                         drop_path_rate=config.model.drop_path)
    elif type == 'swinv2':
...
def load_pretrained(config, model):
    if config.local_rank in [-1, 0]:
        print('-' * 11, f'Loading weight \'{config.model.pretrained}\' for fine-tuning'.center(56),
              '-' * 11)
    ...
    if os.path.splitext(config.model.pretrained)[-1].lower() in ('.npz', '.npy'):
        ...
        if hasattr(model, 'load_pretrained'):
            ...
            if config.local_rank in [-1, 0]:
                print('-' * 18, f'Loaded successfully \'{config.model.pretrained}\''.center(42),
                      '-' * 18)
    ...
    elif type == 'swin' or type == 'swinv2' or type == "mymodel":
        state_dict = checkpoint['model']
        # delete relative_position_index since we always re-init it
        relative_position_index_keys = [k for k in state_dict.keys() if
    ...
    # print(msg)
    if config.local_rank in [-1, 0]:
        print('-' * 16, ' Loaded successfully \'{:^22}\' '.format(config.model.pretrained),
              '-' * 16)
