...
from models.model_utils import print_rank_0, save_sample_map
...
class MultiPartsSampling(nn.Module):
    def __init__(self, dim, input_size, backbone=None, parts_ratio=2, num_heads=16,
        ...
        self.input_size = (input_size // 32, input_size // 32) if backbone_type == 'hier' else (
        ...
        self.block = MultiPartRetrospect(self.dim, self.input_size, parts_ratio, self.num_heads,
                                         att_drop, parts_drop, pos, parts_base, cross_layer,
                                         backbone_type)
    ...
    def feature_weights_pooling(self, x, feature_weights):
        if self.assess:
            ...
            fw_path = join(config.data.log_path, "visualize", "feature_weights")
            os.makedirs(fw_path, exist_ok=True)
            torch.save(x, join(fw_path, "x.pt"))
            torch.save(feature_weights, join(fw_path, "weights.pt"))
        ...
        if self.training and label is not None and self.mixup <= 0:
            ...
            if self.fwp:
                grad = torch.autograd.grad(
                    loss_ce, featmap, torch.ones_like(loss_ce), True)[0].detach()
                gradcam = self.activation(
                    (featmap.detach() * grad.mean(1, keepdim=True)).sum(dim=-1))
                gradcam = self.normalize_cam(gradcam).detach()
                if self.assess:
                    from setup import config
                    fw_path = join(config.data.log_path, "visualize", "feature_weights")
                    os.makedirs(fw_path, exist_ok=True)
                    torch.save(gradcam, join(fw_path, "gradcam.pt"))
...
class MultiPartRetrospect(nn.Module):
    def __init__(self, dim, input_size, parts_ratio=1, num_heads=4, att_drop=0.2, parts_drop=0.2,
        ...
        if self.cross_layer:
            ...
            self.mpsa = PartSamplingAttention(dim, dim, input_size, self.num_parts,
                                              dim // num_heads, parts_base)
        self.activation = nn.GELU()
    ...
    def flops(self):
        flops = 0
        if self.cross_layer:
            for norm, part_generation, mpsa in zip(self.norm_list, self.parts_generation_list,
                                                   self.mpsa_list):
...
class PartSamplingAttention(nn.Module):
    ...
    def forward(self, x, parts):
        ...
        feature_weights = self.softmax2(attention_weights).mean(1) @ parts_attention.reshape(B, -1,
                                                                                             1)

        feature_weights = (feature_weights - feature_weights.min()) / (
                feature_weights.max() - feature_weights.min())
...
if __name__ == '__main__':
    ...
    backbone = swin_backbone(window_size=img_size // 32, img_size=img_size, num_classes=200,
                             cross_layer=True)
    ...
    mps = MultiPartsSampling(dim, img_size, backbone,
                             parts_drop=0.2, parts_base=0., cross_layer=True,
