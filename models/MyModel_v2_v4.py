...
class GCN(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_classes,
                 act_type="GELU", inplace=False, drop_ratio=0., use_norm=False, gat_heads=8,
                 gat_drop=0.5):
        super(GCN, self).__init__()
        self.use_norm = use_norm
        self.conv1 = GATConv(in_channels, hidden_channels, dropout=gat_drop)
        self.conv2 = GATConv(hidden_channels * gat_heads, out_channels,
                             heads=gat_heads, dropout=gat_drop, concat=False)
...
class MyModel(nn.Module):
    def __init__(self, backbone, dim, num_classes, last_size, num_heads,
        ...
        print_rank_0("==================== MyModel_v2(无向图+边权重) ====================")
...
def test():
    ...
    label = torch.randint(200, (batch_size,))
